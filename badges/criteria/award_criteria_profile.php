...
class award_criteria_profile extends award_criteria {
    ...
    public function get_options(&$mform) {
        ...
        $sql = "SELECT uf.id as fieldid, uf.name as name, ic.id as categoryid, ic.name as categoryname, uf.datatype
                ...
                ON uf.objectname = 'user' AND uf.categoryid = ic.id AND uf.visible <> 0
    ...
    public function review($userid, $filtered = false) {
        ...
        if (!empty($infodata)) {
            ...
            $join = " LEFT JOIN {custom_info_data} uid ON objectname = 'user' AND uid.objectid = u.id AND ({$extraon})";
        }
