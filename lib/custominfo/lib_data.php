...
class custominfo_data {
     ...
     * @param  integer  $objectid
     * @param bool $onlyobjectdata True if you only want the ones in $USER.
     * @return  object
     */
    public function get_record($objectid, $onlyobjectdata = true) {
        global $DB;
        ...
        if ($fields) {
            foreach ($fields as $field) {
                $formfield = custominfo_field_factory($this->objectname, $field->datatype, $field->id, $objectid);
                if (!$onlyobjectdata || $formfield->is_object_data()) {
