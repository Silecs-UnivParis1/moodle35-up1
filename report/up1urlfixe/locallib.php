...
function report_up1urlfixe_doublons() {
    ...
    $sql = "SELECT COUNT(DISTINCT cid.id) AS cnt, GROUP_CONCAT(c.shortname) AS names, GROUP_CONCAT(cid.objectid) AS crsid, cid.data "
          ...
          . "JOIN course c ON (c.id = cid.objectid AND cid.objectname = 'course') "
          . "WHERE cid.objectname = :obj AND cif.shortname = :sname "
          . "GROUP BY cid.data HAVING cnt > 1";
    $doublons = $DB->get_records_sql($sql, ['obj' => 'course', 'sname' => 'up1urlfixe']);
...
function report_up1urlfixe_supprimes() {
    ...
    $sql = "SELECT c.shortname AS name, cid.objectid AS crsid, cid.data "
          . "FROM custom_info_data cid "
          . "JOIN custom_info_field cif ON (cid.fieldid = cif.id) "
          . "LEFT JOIN course c ON (c.id = cid.objectid AND cid.objectname = 'course') "
          . "WHERE cid.objectname = :obj AND cif.shortname = :sname AND c.id IS NULL";
    $supprimes = $DB->get_records_sql($sql, ['obj' => 'course', 'sname' => 'up1urlfixe']);
